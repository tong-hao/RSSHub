...
module.exports = async (ctx) => {
  const baseUrl = 'https://www.newsinlevels.com/level/level-3/';
  ...
  const list = $('div.recent-news .news-block')
    ...
    .map((item) => {
      ...
      return {
        ...
        link: `${a.attr('href')}`,
        pubDate: parseDate(item.find('.news-block-right .news-excerpt p').text()),
  ...
  const items = await Promise.all(
    list.map((item) =>
      ctx.cache.tryGet(item.link, async () => {
        ...
        const [rate, unknowWords] = words.wordsRate(`${post.html()}`);
